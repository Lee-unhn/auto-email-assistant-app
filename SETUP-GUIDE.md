# 使用者版安裝清單（一人一台，你幫他們設定一次）

目標：使用者零操作 —— 每天自動讀信 → 自動排進他們的 Google 行事曆 → 每天收一封摘要 email，要看細節再開 app。

## 每位使用者的電腦，你做這幾步（約 10 分鐘）
...
3. **在「設定」頁輸入（全部 app 內完成，不必編輯任何檔案）**：
   - **AI 金鑰**：他的免費 Gemini 金鑰（aistudio.google.com → Get API key）貼進「Gemini API Key」→ 按「存」。（使用者電腦多半沒 Claude Code，用免費 Gemini；佇列+重試會吸收免費限流。）
...
4. **行事曆（預設＝app 私密行事曆，無需任何設定）**
   ...
   - **（選用）同時寫進 Google 行事曆** —— 只有「不介意公開/非公開行事曆」的使用者才開：
...
## 引擎自動判斷
`run-assistant.ps1` 會自己偵測：這台有 Claude Code 就用 Claude，沒有就用 Gemini。所以同一套在你的電腦（Claude）和使用者電腦（Gemini）都直接能跑。
